...
public class CarvableHelper {
	...
	public void addVariation(String inputDescription, int metadata, String texture, Block block, int blockMeta) {
		...
		if(blockName==null && block!=null) blockName=block.translateBlockName(); // TODO - Fyber - getLocalizedName
...
//			return null;
	...
	void register(Block block, String name, Class cl) {
		...
		for (CarvableVariation variation : variations) {
			...
			if (block instanceof BlockMarbleSlab && ((BlockMarbleSlab) block).isBottom) {
				...
				// TODO - Fyber - Make fields visible
	...
	public void registerIcons(String modName, Block block, IconRegister register) {
		for (CarvableVariation variation : variations) {
			if (variation.block != null) {
				RetroUtil.registerIcons(variation.block, register);

				if (variation.block instanceof BlockPane) {
					variation.icon = RetroUtil.getBlockTextureFromSide_FromAtlas(variation.block, 2);
					// TODO - Fyber - Fix this
					...
					switch (variation.kind) {
					case 0:
						variation.icon = RetroUtil.getIcon(variation.block, 2, variation.blockMeta);
						break;
					case 1:
						variation.icon = RetroUtil.getIcon(variation.block, 2, variation.blockMeta);
						variation.iconTop = RetroUtil.getIcon(variation.block, 0, variation.blockMeta);
						break;
					case 2:
						variation.icon = RetroUtil.getIcon(variation.block, 2, variation.blockMeta);
						variation.iconTop = RetroUtil.getIcon(variation.block, 1, variation.blockMeta);
						variation.iconBot = RetroUtil.getIcon(variation.block, 0, variation.blockMeta);
						break;
					}
				}
			} else {
				...
				case 4:
					...
					break;
				case 5:
					//System.out.println("CTMH: " + modName + " " + block + " " + block.getBlockName());

...
public class EntityCloudInABottle extends EntityThrowable {
	...
	public EntityCloudInABottle(World par1World, EntityLiving par2EntityLiving) {
	...
	protected void onImpact(MovingObjectPosition movingobjectposition) {
		...
		// worldObj.playSound(this.posX, this.posY, this.posZ, "random.glass",
		// 1.0F, worldObj.rand.nextFloat() * 0.1F + 0.9F, false);
				...
				for (int j2 = x; j2 < x + random.nextInt(4) + 1; j2++) {
					for (int k2 = y; k2 < y + random.nextInt(1) + 2; k2++) {
						for (int l2 = z; l2 < z + random.nextInt(4) + 1; l2++) {
							if (world.getBlockId(j2, k2, l2) == 0 && Math.abs(j2 - x) + Math.abs(k2 - y) + Math.abs(l2 - z) < 4 * 1 + random.nextInt(2)) {
								world.setBlock(j2, k2, l2, Chisel.blockCloud.blockID);
								count++;
							}
						}
					}
